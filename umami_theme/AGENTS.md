# UmamiTheme Theme - Agent Rules

This document contains coding rules and conventions for the UmamiTheme theme that AI agents should follow when making changes.
...
**Note**: Even though Drupal core templates sometimes use `<div{{ attributes }}>`, this pattern is not allowed in UmamiTheme theme components.
...
```twig
{% include '@umami_theme/components/icon/icon.twig' with {
  weight: 'bold',
  icon: icon
} %}
```
...
**Rule**: After completing any changes to the UmamiTheme theme, always run `npm run format` and `npm run build` to ensure code is properly formatted and the build artifacts are up to date.
...
**Note**: These commands should be run from the UmamiTheme theme directory (`web/themes/custom/umami_theme/`). Running these commands ensures that:
